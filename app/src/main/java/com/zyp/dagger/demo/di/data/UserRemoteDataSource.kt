/*
 * Copyright (C) 2019 NaLong. All Rights Reserved.
 * NaLong group reserve all right of the client.
 * Without authorization, no individual or organization may copy, extract or modify it.
 * If you find any infringement, please contact us.
 */
package com.zyp.dagger.kotlin.repository.data

import com.zyp.dagger.demo.http.api.LoginRetrofitService
import javax.inject.Inject

/**
 * 描述：
 * 作者：@author alex
 * 创建时间：2021/6/11 3:29 下午
 */
class UserRemoteDataSource @Inject constructor(private val service: LoginRetrofitService) {

}